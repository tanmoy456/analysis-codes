...
gd_values = ["0.01", "0.005", "0.0025", "0.001", "0.0005", "0.00025", "0.0001", "0.00005", "0.000025"]
...
for id in tqdm(range(len(prop)), desc="Processing properties"):
    ...
    for gd in gd_values:
        if id==0:
            base_dir = f"{directories[0]}/" + prop_name + f"_en_avg_gd_{gd}.txt"
        else:
        ...
        try:
            ...
            if itt == 0:
                ...
                plt.plot(time*float(gd), values, marker=markers[marker_idx % len(markers)], linestyle='-', label=label)
    ...
    if id == 0:
        ...
        plt.ylabel(r"$\chi_{4}(t)$")
        if itt==1:
            plt.xlim(1e-5,1.0)

    plt.legend(fontsize=8, title=f"$a={a}$")  # Smaller legend for clarity
    plt.grid(True, linestyle="--", linewidth=0.5, alpha=0.6)

    # Save the figure
    plt.savefig(f"{directories[1]}/{prop_name}_a_{a}_x_{polt_x[itt]}.png", dpi=200,
