...
for en in tqdm(range(1, n_ensembles + 1)):
    ...
    for i in range(0, n_origins):
        ...
        MSD_x += msd_x.mean(axis=-1) # mean over ncell 
        MSD_y += msd_y.mean(axis=-1)

        # Correlation function (Qs)
        #qt_x = np.sin(k*msd_x)/(k*msd_x)
        #qt_y = np.sin(k*msd_y)/(k*msd_y)
        qt_x = np.cos(k*msd_x)
        qt_y = np.cos(k*msd_y)
    ...
    # Origin average
    MSD_x /= n_origins  # mean over time origins
    MSD_y /= n_origins
    Qt_x /= n_origins
    Qt_y /= n_origins
    Qt2_x /= n_origins
...
ensemble_avg_msd_x /= n_ensembles # mean over time ensembles
